...
local AllBodyParts = {
    'HumanoidRootPart', 'LeftHand', 'RightHand', 'LeftLowerArm', 'RightLowerArm', 'LeftUpperArm',
    'RightUpperArm', 'LeftFoot', 'LeftLowerLeg', 'UpperTorso', 'LeftUpperLeg', 'RightLowerLeg',
    'RightUpperLeg', 'LowerTorso', 'RightFoot', 'Head'
}
...
local function getPredictedPosition(part, velocity, playerState)
    if part then
        ...
        local smoothedVelocity = velocity * predictionMultiplier * timeToPredict
        
        -- Adjust predictions based on state of player movement
        if playerState == "Jumping" then
            smoothedVelocity = smoothedVelocity + Vector3.new(0, 10, 0)  -- Add upward movement prediction
        elseif playerState == "Crouching" then
            smoothedVelocity = smoothedVelocity * 0.5  -- Slower horizontal movement for crouching
        elseif playerState == "Walking" then
            smoothedVelocity = smoothedVelocity * 1.25  -- Adjust for normal walking
        elseif playerState == "Flying" then
            smoothedVelocity = smoothedVelocity * 2  -- Increased movement speed in all directions
        end

        return part.Position + smoothedVelocity
    end
    return nil
end
...
local function getPlayerState(humanoid)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if humanoid and humanoid.Health > 0 then
                ...
                local playerState = getPlayerState(humanoid)

                for _, partName in ipairs(AllBodyParts) do
                    local part = player.Character:FindFirstChild(partName)
                    if part then
                        local predictedPosition = getPredictedPosition(part, part.Velocity, playerState)
                        if predictedPosition and isWithinBoxFOV(predictedPosition) and isInLineOfSight(part) and os.clock() - lastClickTime >= getgenv().Forbidden.TriggerBot.Settings.Cooldown then
